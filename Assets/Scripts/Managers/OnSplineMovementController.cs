...
public class OnSplineMovementController : MonoBehaviour
{
    ...
    private void UpdateMove()
    {
        ...
        _velocity = Mathf.Clamp(_velocity + _direction * Time.deltaTime* accelCurve.Evaluate(AccelerationCurveT) , -_maxSpeed, _maxSpeed);
        AccelerationCurveT += Time.deltaTime;
        ...
        if (_direction == 0f)
        {
            ...
            {
                accel = deceleration;
                _velocity = Mathf.Clamp(_velocity + decelCurve.Evaluate(AccelerationCurveT) * -(Mathf.Sign(_velocity)) * Time.deltaTime, -_maxSpeed, _maxSpeed);
                AccelerationCurveT += Time.deltaTime;
            }
            else
            {
